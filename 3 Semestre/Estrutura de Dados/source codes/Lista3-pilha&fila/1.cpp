...
void PushRight(Pilha ** pilha, INFO info){
	InserirFinalLLEncDup(pilha, info);
}

NODO * PopRight(Pilha ** pilha){
	return RemoverFinalLLEncDup(pilha);
}

void PushLeft(Pilha ** pilha, INFO info){
	InserirFinalLLEncDup(pilha, info);
}

NODO * PopLeft(Pilha ** pilha){
	return RemoverFinalLLEncDup(pilha);
}
...
int main() {
	...
	//preciso verificar se eu removo o ultimo elemento de determinada pilha pra n ferrar a outra
