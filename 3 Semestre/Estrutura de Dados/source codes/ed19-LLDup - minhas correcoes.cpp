...
void MostrarLLEncDupInv(NODO *LLDup) {
    NODO * aux;
	aux = ObterUltimoNodoLLEndDup(LLDup);
	cout << "/";
	while (aux != NULL) {
		cout << "<->" << aux->info;
        aux = aux->ant;
...
int main() {
	...
	cin >> info;
	while(info != 0){
		InserirInicioLLEncDup(&LLDup, info);
		cin >> info;
	}
	
	RemoverInicioLLEncDup(&LLDup);
	RemoverFinalLLEncDup(&LLDup);
	
	MostrarLLEncDupInv(LLDup);
