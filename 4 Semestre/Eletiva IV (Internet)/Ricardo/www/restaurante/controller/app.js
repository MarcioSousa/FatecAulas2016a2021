 ...
 **/
var app = angular.module('restauranteApp', ['angularUtils.directives.dirPagination', 'ngRoute', 'ngMessages', 'ngCookies', 'ngMask', 'naif.base64', 'ngSanitize', 'ui.select'])
...
app.controller('restauranteController',
        function ($scope, $http, $window, $rootScope, Configuracoes, $cookies) {
             ...
             * 
             **********************************************************/
//            var urlBase = 'http:/estoque/api';
            var urlBase = 'http:/restaurante/api';
            /*********************************************************
             *              
             */
            ...
            $scope.imprimeDiv = function (nomeDIV, nomeTitulo) {
                ...
                popupWin.document.write('<html><head><title>Relatorio</title><link rel="stylesheet" type="text/css" href="css/bootstrap.min.css"/></head>' +
                        '<body onload="window.print()"><img src="img/logo.jpg"><h1>Sistema de Controle de Restaurante</h1><h2>' + nomeTitulo + '</h2>' + printContents + '</html>');
            ...
            $scope.validaLogin = function (login) {
                $http.get(urlBase + "/login/" + login.usuario + "/" + login.senha)
                        .success(function (data) {
                            if (data == 'true') {
                                var urlInicial = "http://" + $window.location.host + "/restaurante/#/menu";
            ...
            $scope.limpaProduto = function (nomeForm) {
                ...
                nomeForm.fabricacao.$touched = false;
            ...
            $scope.obterProdutoPeloId = function (produto) {
                $http.get(urlBase + "/produtos/" + produto.codigo)
                        .success(function (data) {
